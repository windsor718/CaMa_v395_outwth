...
D2FLDHGT(:,:,:)=0._JPRB
D2RIVMAN(:,:)  =0._JPRB
D2RIVSHP(:,:)  =0._JPRB     !! processed later if not LRIVSHP
D2RIVBTA(:,:,:)=0._JPRB
D2MEANSL(:,:)  =0._JPRB
D2DWNELV(:,:)  =0._JPRB
D2GDWDLY(:,:)  =0._JPRB
...
IF ( LFPLAIN .AND. .NOT. LRIVSHP) THEN
  D2RIVSTOMAX(:,:) = D2RIVLEN(:,:) * D2RIVWTH(:,:) * D2RIVHGT(:,:)
ELSE IF ( LFPLAIN .AND. LRIVSHP ) THEN
  DO ISEQ=1, NSEQMAX
...
IF( LRIVSHP )THEN
  ...
  OPEN(TMPNAM,FILE=TRIM(CRIVSHP),FORM='UNFORMATTED',ACCESS='DIRECT',RECL=4*NX*NY)
  READ(TMPNAM,REC=1) R2TEMP(:,:)
    IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
  ...
  OPEN(TMPNAM,FILE=TRIM(CRIVBTA),FORM='UNFORMATTED',ACCESS='DIRECT',RECL=4*NX*NY)
  DO ILFP=1,4
    READ(TMPNAM,REC=ILFP) R2TEMP
    IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
    CALL MAP2VEC(R2TEMP,D2TEMP)
    D2RIVBTA(:,:,ILFP)= D2TEMP(:,:)
  ENDDO
  CLOSE(TMPNAM)
ENDIF
